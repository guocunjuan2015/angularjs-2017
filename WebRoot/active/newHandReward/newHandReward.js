...
if (browser.versions.ios == true) {
	...
	$(".checkbtn").on("touchend", function(){
    	...
    	/*注册*/
    	$.ajax({
    	   url:localPath + "/offLineActivity/register",
    	   type:"post",
    	   data:{
    		   mobile:$("#m").val(),
    	   ...
    	   success:function(data){
    		   ...
    		   if (data.success) {
    			   ...
    			   $(".registerBox").hide();
    			   $(".nextRgister").show();
    			/*   $("#viewGift").attr("href", basePath + "/sys/user/award/myGift.jsp?from=app&userId=" + data.userId + '&lotteryName=奖品');*/
    			   $("#viewGift").attr("href", basePath + "/sys/user/award/myGift.jsp?from=app&userId=" + data.userId +"#/prize");
    		   } else {
    			   $("#prompt_txt").text(data.msg);
    		   }
    	  	},
    	...
    	/*function lotteryBarbecue (userId) {
    		$.ajax({
    	    	  url:localPath + "/lotteryAward/lotteryBarbecueResp",
    	    	  type:"post",
    	    	  data:{
    	    		  userId:userId
    	    	  },
    	    	  ...
    	    	  success:function(data){
		        	if (data.status == "003") {
		        		$("#prompt font").text(data.voucher_lotteryName);
		        		$("#prompt a").attr("href", basePath + "/sys/user/award/myGift.jsp?userId=" + userId);
		        		$("#prompt2 font").text(data.lotteryName);
		        		...
		        		$(".downLoad").show();
		        		$(".barbecueMeal_txt").css({"margin-bottom":"4rem"});
		        	} else {
		        		alert(data.msg + "22222");
		        	}
    	    	 },
    	    	 error:function() {
    	    		alert("服务器错误，请联系管理员！");
