...
function checkStatus() {
	/* document.getElementById("footpanel").scrollIntoView(); */
	$.ajax({
				type : "POST",
...
$http.post("project/getLoanDetail.do",data,postCfg).success(function(data){
		...
		$scope.proofUrlLists = [];/*证明材料*/
        if($scope.proofUrlLists.length > 0){
        	$scope.proofUrlLists = result.resultObject.proofUrlList; /*证明材料*/
		} 
