...
$(function() {
	...
	/*判断优惠券的类型 1、红包 2、加息券*/
	if (storage.getItem("couponType") == '1') {
			//红包
			$("#coupon").text(storage.getItem("couponAmount") + "元红包");
			$("#couponId").val(storage.getItem("couponId"));
			storage.removeItem("couponId");
         	$("#error_info").text();
         	$("#coupon").css("color","#ff5555");
         	$("#error_info").text("使用红包之后不能进行债权转让");
	} else if (storage.getItem('couponType') == '2') {
			// 加息券
			$("#coupon").text(storage.getItem("couponAmount") +"%加息券");
			$("#couponId").val(storage.getItem("couponId"));
			storage.removeItem("couponId");
         	$("#coupon").css("color","#ff5555");
         	$("#error_info").text("");
...
function sureUse() {// 仍然使用
	...
	case 3:
		if (storage.getItem("ticketId")) {
			...
			storage.removeItem("moreRateId");
			storage.removeItem("moreRateValue");
		} else if (storage.getItem("couponId")) {
			storage.removeItem("couponId");
			storage.removeItem("couponValue");
		}
...
function umpInvest() {
	...
	var couponType = getRequestParam("couponType");/*类型  1、红包 2、加息券*/
	var money = $("#invest_number").val();
	var couponId = "";
	var moreRateId = "";
	if(couponType == "1"){
		couponId = getRequestParam("couponId");
	} else {
		var moreRateId = getRequestParam("couponId");
	}
	var ticketId = "";
	if (storage.getItem("couponId")) {
		couponId = storage.getItem("couponId");// 代金券ID
	}
	if (storage.getItem("ticketId")) {
		ticketId = storage.getItem("ticketId");// 投资券ID
	}
	if (storage.getItem("moreRateId")) {
		moreRateId = storage.getItem("moreRateId");// 投资券ID
	}
	if (storage.getItem("redEvpId")) {
		couponId = storage.getItem("redEvpId");// 红包ID
	}
...
function umpTransferInvest() {
	...
	if (window.localStorage.getItem("ticketId")) {
		ticketId = window.localStorage.getItem("ticketId");// 投资券ID
	}
	if (storage.getItem("redEvpId")) {
    ...
    $http.post("project/getLoanDetail.do",data,postCfg).success(function(data){
		...
		function couponOrMorerateInterest(investMoney) {
			...
			if (storage.getItem("couponType") == '1') {
				...
				cOrMInterest = storage.getItem("couponAmount");
				storage.removeItem("couponAmount");
			} else if (storage.getItem('couponType') == '2') {
				storage.removeItem("couponType");
				// 加息券
				var morerate = storage.getItem("couponAmount");
				storage.removeItem("couponAmount");
				if (($scope.loanType == '111' && $scope.repaymentWay == '301') || $scope.repaymentWay == '306' || $scope.repaymentWay == '307') {
