...
$(function() {
	...
	var feeRate = Number($("#feeRate").val());//手续费利率
	$("#addWithdrawInfo li").click(function(){
		...
		withdrawCash = $(".withdrawCash a").text(Number($(this).find("input").val()));
		$(".withdrawCash font").text("元提现券");
		if(cash_money < 2){
		   	$("#withdrawErr").text('请输入正确的提现金额。');
			if (feeRate == 0.00) { //首次提现免提现费
				//fee = Number("0.00");
...
function calculateWithdrawCash(){
	...
	var withdrawCash = Number($(".withdrawCash a").text());//选中提现券之后展示的提现券值
	...
	if (withdrawCash > 0) {
		if ((fee - withdrawCash) < 0) {
			fee = Number("0.00");
		} else {
			fee = (fee - withdrawCash).toFixed(2);
		}
	}
	$("#fee").text(fee);
		...
		success : function(response) {
			...
			if (info.result =="1") {
				...
				if (!isEmptyStr(data)) {
					...
					var maxVal=Number(arr[0]);
					for(var i=0;i<arr.length;i++){
						_data = data[i];
						if(maxVal<Number(arr[i])){
							maxVal=Number(arr[i]);
						}
					};
					$.each(data,function(index,obj){
						...
						/*if (maxVal == Number(obj.amount)) {
							str += '<li class="withdrawCheck w_checked" style="color:#ff5555;">';
							str += '<span>'+ obj.validity +'</span>'+ obj.amount +'元提现劵';
							str += '<input id="'+ obj.couponId +'" name="coupon" type="checkbox" checked="checked" value="'+ obj.amount +'" class="couponinput" >';
					...
					$(".withdrawCash font").text("提现券");
					$(".withdrawCash font").css("float","right")
						...
						str += '</li>';
						$(".withdrawCash a").css({"text-decoration":"none","color":"#999","pointer-events":"none"});
						$(".withdrawCash a").text("无提现券可用");
						$(".withdrawCash").removeAttr("onclick","openCheck()");
				}
