...
$(document).ready(function() {
	$("#step").click(function() {
		...
		if (checkM("m") && checkP("p") && checkVCIsExist() && checkC("c")) {
			if($("#utm_source").val() =='2017' || $("#utm_source").val() =='2018' || $("#utm_source").val() =='2019'|| $("#utm_source").val() =='2021' || $("#utm_source").val() =='2022'|| $("#utm_source").val() =='2025'|| $("#utm_source").val() =='2029'|| $("#utm_source").val() =='2029'|| $("#utm_source").val() =='2030'
				|| $("#utm_source").val() =='2031'|| $("#utm_source").val() =='2032' || $("#utm_source").val() =='2033' || 
				$("#utm_source").val() =='2080'|| $("#utm_source").val() =='2081'|| $("#utm_source").val() =='2082'|| $("#utm_source").val() =='2083'|| $("#utm_source").val() =='2084'	|| $("#utm_source").val() =='2085'	|| $("#utm_source").val() =='2086' || $("#utm_source").val() =='2087'
				 || $("#utm_source").val() =='2089'	|| $("#utm_source").val() =='2091'|| $("#utm_source").val() =='2093'		
				...
				checkUser( basePath + '/download/download.jsp');
			}else{
				checkUser( basePath + "/register_success.jsp");
			}
...
$(document).ready(function() {
	$("#finish").click(function() {
		...
		// 验证用户，手机号，密码，图形验证码，手机验证码
		if (checkM("m") && checkP("p") && checkVCIsExist() && checkC("c")) {
			checkUser( basePath + "/personal_center.jsp");
		} else {
			$("#step").attr("disabled", false);
			$("#finish").attr("disabled", false);
		}
		...
		$.ajax({
			...
			success : function(response) {
				...
				if (res == 1) { 
					...
					}  else if ($("#active").val() != null && $("#active").val() == "inviting") {
						window.location.href = "/rqb/active/inviting/inviting.jsp?userId=" + userId;
					}  else if ($("#active").val() != null && $("#active").val() == "nationalDay") {
						window.location.href = "/rqb/active/nationalDay/nationalDay.jsp?userId=" + userId;
					}   else if ($("#active").val() != null && $("#active").val() == "highPurchase") {
						window.location.href = "/rqb/active/highPurchase/highPurchase.jsp?userId=" + userId;
					}   else if ($("#active").val() != null && $("#active").val() == "decActive") {
						window.location.href = "/rqb/active/decActive/decActive.jsp?userId=" + userId;
					}   else if ($("#active").val() != null && $("#active").val() == "hamster") {
						window.location.href = "/rqb/active/hamster/hamster.jsp?userId=" + userId;
					}else {
						if (isEmptyStr(url)) {
...
function checkVCIsExist() {
	var flag = false;
	var url = localPath +"/member/validateCode?"+new Date().getTime();
...
function getRadioCode() {
	$.ajax({
		type : "POST",
		url : basePath + "/register/getRadioCode.do",
		data : {
			m : $("#m").val()
		},
		success : function(response) {
...
function onBoxClicked() {
	if ($('#agreeBox').attr("checked") == 'checked') {
		...
		$(".squaredTwo").removeClass("bg");
		$("#step").attr("disabled", true);
	} else {
		$('#agreeBox').attr('checked',true);
		$(".squaredTwo").addClass("bg");
		$("#step").attr("disabled", false);
	}
}
